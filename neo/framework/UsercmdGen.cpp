...
void idUsercmdGenLocal::AdjustAngles( void ) {
	...
	if ( toggled_run.on ^ in_alwaysRun.GetBool() ) { //stradex: in_alwaysRun works with SP now
...
void idUsercmdGenLocal::InitCurrent( void ) {
	...
	cmd.buttons |= in_alwaysRun.GetBool() ? BUTTON_RUN : 0; //stradex: in_alwaysRun works with SP now
