...
void idActor::SetupHead( void ) {
	...
	if ( gameLocal.isClient && !gameLocal.mpGame.IsGametypeCoopBased()) { //head joints in coop
		return;
	}
	...
	if ( headModel[ 0 ] ) {
		...
		headEnt = static_cast<idAFAttachment *>( gameLocal.SpawnEntityType( idAFAttachment::Type, &args , true) ); // COOP: In coop we can spawn heads
